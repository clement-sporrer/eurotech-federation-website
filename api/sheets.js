// Vercel API Function for fetching Google Sheets data
...
export default async function handler(req, res) {
  ...
  // Get sheet name from URL (e.g., /api/sheets?sheet=Fellows)
  const sheetName = req.query.sheet;
  
  // If no sheet name provided
  if (!sheetName) {
    return res.status(400).json({ error: 'Missing sheet name parameter' });
  }
